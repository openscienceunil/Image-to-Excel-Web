...
def make_table():
    if request.method == 'POST':
        ...
        # ici choisir ce qu'on veut annalyser comme tableau
        # (min et max pour les boite)...
        ...
        dico_text, nb_column, list_text = write_text_from_panels(
            img_array,
            panel_boxes
        )
...
def upload():
    ...
    ret_data['value'] = "L'image '" + path_name + \
    "' a ete telechargee en tableau excel sur le bureau !"
...
def choose_img():
    ...
    img_tableau_noir = img.point(lambda i: (i-127)*2.3)\
                          .point(lambda i: 0 if i < 250 else 255)
    ...
    for i in range(len(tot_pixels_col)):
        if path_end == '.png':
            if tot_pixels_col[i].count((0,0,0,255)) < moy:
                tot_pixels_col[i] = [(255, 255, 255, 255)] * \
                len(tot_pixels_col[i])
        elif path_end =='.jpg':
...
def purifier_line_tableau(im, path_end):
...
def find_objects_with_size(
    ...
    panel_boxes = []
    for boxe in boxes:
        if min_height < boxe[0].stop - boxe[0].start or min_height == \
        None: # hauteur min
            if max_height > boxe[0].stop - boxe[0].start or max_height == \
            None: # hauteur max
                if min_width < boxe[1].stop - boxe[1].start or min_width == \
                None: # largeur min
                    if max_width > boxe[1].stop - boxe[1].start or max_width ==\
                    None: # largeur max
...
def write_text_from_panels(image_array, panels):
    ...
    for i in range(len(img_content)-nb_column):
        ...
        if  i%(nb_column) != 0 :
            if '-|' in img_content[i] or 'O' in img_content[i] or \
            ')' in img_content[i] or '-1-' in img_content[i] or \
            '- -' in img_content[i] or '(' in img_content[i] or \
            '+' in img_content[i] or ',' in img_content[i] or \
            ' ' in img_content[i]:
                img_content[i]=img_content[i].replace('-|','1')\
                                             .replace('-1-','')\
                                             .replace('- -','-')\
                                             .replace('O','0')\
                                             .replace('(','')\
    ...
    for i in range(len(img_content)):
        img_content[i]=img_content[i].replace("\xc3","")\
                                     .replace("\xa9","")\
                                     .replace("\xe2","")\
                                     .replace("\x80","")\
                                     .replace("\x99","")\
